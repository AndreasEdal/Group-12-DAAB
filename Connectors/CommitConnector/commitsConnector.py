...
spark = SparkSession.builder.appName('commits-connector') \
...
schema = StructType([
    ...
    StructField("commitNumber", IntegerType())
])
...
df = spark \
    ...
    .option("subscribe", "answerCommitFreq") \
    .load()


value_df = df.select(from_json(col("value").cast("string"),schema).alias("value"))

selected_df = value_df.selectExpr('value.repo_name', 'value.commitNumber')
